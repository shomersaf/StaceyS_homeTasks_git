...
function init() {
  ...
  limitSpan.innerText = " Limit: ";
  const skipSpan = document.createElement("span");
  skipSpan.innerText = " Skip: ";
  const nextButton = document.createElement("button");
  nextButton.innerText = "next";
  nextButton.addEventListener("click", skipNextTen);
  prevButton.addEventListener("click", skipPrevTen);

  function skipNextTen (){
    
    currentState.skip += 10;
    getProductsHandler(currentState.limit, currentState.skip);
  }
  function skipPrevTen (){
    if(currentState.skip >=10){
      currentState.skip -= 10;
    }
  ...
  DOM.controllers.append(prevButton, limitSpan, selectOption, skipSpan, selectOption2, nextButton);
